...
const Dashboard = () => {
	...
	useEffect(() => {
		...
		const fetchConversations = async () => {
			const res = await fetch(`https://chat-n8pa.onrender.com/api/conversations/${loggedInUser?.id}`, {
				method: 'GET',
				headers: {
	...
	useEffect(() => {
		const fetchUsers = async () => {
			const res = await fetch(`https://chat-n8pa.onrender.com/api/users/${user?.id}`, {
				method: 'GET',
				headers: {
					'Content-Type': 'application/json',
				}
	...
	const fetchMessages = async (conversationId, receiver) => {
		const res = await fetch(`https://chat-n8pa.onrender.com/api/message/${conversationId}?senderId=${user?.id}&&receiverId=${receiver?.receiverId}`, {
			method: 'GET',
			headers: {
				'Content-Type': 'application/json',
			}
	...
	const sendMessage = async (e) => {
		...
		const res = await fetch(`https://chat-n8pa.onrender.com/api/message`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
	...
	return (
		<div className='w-screen flex'>
			<div className='w-[25%] h-screen bg-secondary'>
				<div className='flex items-center my-5 mx-14 h-[15%]'>
					<div>
						<img src={Img5} width={65} height={50} className='border border-primary p-[2px] rounded-full' />
					</div>
					<div className='ml-8'>
				...
				<div className='px-14 h-[75%] overflow-scroll'>
					<div className='mt-3 text-primary text-lg'>Messages</div>
					<div>
						{
			...
			<div className='w-[50%] h-screen bg-white flex flex-col items-center'>
				{
					...
					<div className='w-[100%] bg-secondary h-[70px] flex items-center px-14 py-2'>
				...
				<div className='h-[82%] w-full overflow-scroll shadow-sm'>
					<div className='p-14'>
						{
			...
			<div className='w-[25%] h-screen bg-light px-8 py-5 overflow-scroll'>
